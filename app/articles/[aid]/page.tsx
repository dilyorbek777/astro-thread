...
type Props = { params: { aid: string } }

export const generateMetadata = async ({ params }: Props): Promise<Metadata> => {
    const res: News = await fetch('https://api.spaceflightnewsapi.net/v4/articles/' + params.aid, { cache: "no-store" }).then(d => d.json())
    ...
    return {
        ...
        metadataBase: new URL(`https://astro-thread.vercel.app/articles/${params.aid}`),
        ...
        openGraph: {
            ...
            description:
                `${res.summary}`,
            type: 'website',
            url: `https://astro-thread.vercel.app/articles/${params.aid}`,
            locale: 'en_EN',
...
export default async function Aid({ params }: Props) {

    const data: News = await fetch(`https://api.spaceflightnewsapi.net/v4/articles/${params.aid}`).then(r => r.json())
